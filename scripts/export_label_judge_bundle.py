...
CANDIDATES = [
    Candidate(
        ...
        relative_dir="jigsaw_sample_comments_400_hierarchical_8_40_refine_none",
        note="`merge_labelling` の結果をそのまま使う baseline。",
    ),
    Candidate(
        key="setwise",
        label="setwise_refine",
        relative_dir="jigsaw_sample_comments_400_hierarchical_8_40_refine_setwise",
        note="sibling 全体を見て代表性と重複抑制を両立させる最初の refinement。",
    ),
    Candidate(
        key="contrast",
        label="contrast",
        relative_dir="jigsaw_sample_comments_400_hierarchical_8_40_refine_contrast",
        note="sibling 差分を前半に出し、短くしつつ意味差を残す prompt variant。",
    ),
    Candidate(
        key="balanced",
        label="balanced",
        relative_dir="jigsaw_sample_comments_400_hierarchical_8_40_refine_balanced",
