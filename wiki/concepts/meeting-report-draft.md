...
5. 現在の open PR は `#887` と `#863` の 2 本です。`#887` は `#886` を close する Plotly scattergl CSP 修正で checks は green。壊れた理由は旧レポート schema ではなく、`PR #848` で入った production CSP が `scattergl` / regl の runtime eval 要件を満たしていなかったためで、dev / static E2E では条件が再現していませんでした。`#863` は `#731` Windows setup 文字化け対応で実機確認と review 判断待ち。`PR #883` は 2026-05-31 に撤回済みで、新方針による全文草案を [[pr-883-developer-quickstart-draft-2026-05-31]] に置き、Issue `#876` 本体にも追加要件と完了条件を反映済み。
...
- `PR #887` / Issue `#886`: 本番 public-viewer の report URL では CSP が `script-src 'self' 'unsafe-inline'` のままで、Plotly `scattergl` が使う `@plotly/regl` の runtime code generation が CSP に止められ、`.no-webgl` overlay が前面に残っていた。`next dev` は `unsafe-eval` が許可され、static E2E は CSP header を返さないため既存 CI では早期検知できなかった。次は production `next start` smoke で `.no-webgl` 非表示・CSP violation なし・CSP header の `unsafe-eval` opt-in を確認するのが筋。詳細は [[issue-887-scattergl-csp-regression-2026-06-01]]。
