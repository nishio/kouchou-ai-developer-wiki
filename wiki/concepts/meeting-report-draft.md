...
- `PR #887` / Issue `#886`: 2026-06-01 に merge され Azure Deployment workflow も success になったが、当初 stable URL は旧 CSP のままで `.no-webgl` overlay も visible。`public-viewer--0000166` は startup `next build` の TypeScript phase で `Killed` / exit 137 を起こし、一時 `Unhealthy / Degraded` だった。その後 2026-06-01 19:41 JST に `next start` まで到達し、19:44 JST 確認時点では latestReady も `0000166`、stable URL も `unsafe-eval` 付き CSP。deploy success false positive は `#887` 固有ではなく、少なくとも `#851` 以降の successful deploy で `latestReadyRevisionName` が旧 revision のままでも stable URL 200 で success になっていた。workflow 側は latest revision readiness と representative report smoke を見るように直す必要がある。runtime build exit 137 risk は残るため、memory increase または image build 時 `.next` 化も検討。詳細は [[issue-887-scattergl-csp-regression-2026-06-01]]。
