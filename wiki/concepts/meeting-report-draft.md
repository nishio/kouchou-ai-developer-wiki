...
5. 現在の open PR は `#863` の 1 本です。`#887` は merge され Azure Deployment も success になりましたが、本番 stable URL はまだ旧 CSP (`script-src 'self' 'unsafe-inline'`) を返し、`.no-webgl` overlay も visible のままです。Azure logs では `public-viewer--0000166` の startup `next build` が TypeScript phase で `Killed`、exit 137 になっており、ACA は `Unhealthy / Degraded` で旧 ready revision のままです。`#863` は `#731` Windows setup 文字化け対応で実機確認と review 判断待ち。`PR #883` は 2026-05-31 に撤回済みで、新方針による全文草案を [[pr-883-developer-quickstart-draft-2026-05-31]] に置き、Issue `#876` 本体にも追加要件と完了条件を反映済み。
...
- `PR #887` / Issue `#886`: 2026-06-01 に merge され Azure Deployment workflow も success になったが、本番 stable URL は旧 CSP のままで `.no-webgl` overlay も visible。`public-viewer--0000166` は `Unhealthy / Degraded`、`Deployment Progress Deadline Exceeded. 0/1 replicas ready.`。console log では startup `next build` が compile 成功後の TypeScript phase で `Killed`、system log では startup probe failure 連続と exit 137。次は memory increase で emergency recovery するか、runtime build をやめて image build 時に `.next` を作る修正に進む。workflow 側も latest revision readiness と representative report smoke を見るように直す。詳細は [[issue-887-scattergl-csp-regression-2026-06-01]]。
