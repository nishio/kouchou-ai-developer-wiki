...
- 2026-05-22: 進行中: `#860 -> draft PR #862` の実機 E2E が `public-viewer` の `../shared/csp` 欠落を検出したため、`apps/public-viewer` と `apps/static-site-builder` の Dockerfile に `apps/shared` を含める修正を追加。Windows 実機の Docker Desktop で両 image build が通ることを確認し、PR branch に push 済み。[[source-code]]より [[github-dev-docs]]より
