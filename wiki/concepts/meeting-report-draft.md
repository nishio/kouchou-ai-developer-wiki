...
7. Windows setup `#877` は、Docker Desktop が使える Windows 10/11 を標準入口にし、組織ポリシー / ライセンスで Docker Desktop や WSL2 が使えない端末は beginner guide のサポート境界外として明示する方針にしました。加えて tokoroten / nishio の「単一実行バイナリ」議論を受け、Node runtime を build-time assets に閉じ込めて Python/FastAPI + static assets runtime へ寄せる前提 issue `#885` を起票。MVP は external API route と、API 契約不要の offline route の 2 本比較に修正し、offline route には同梱モデルだけでなく Foundry Local / Chrome built-in AI / Windows AI APIs の native runtime 候補も入れました。[[issue-877-windows-setup-guide-scope]]より [[node-runtime-free-windows-exe-2026-05-31]]より
