...
7. Windows setup `#877` は、Docker Desktop が使える Windows 10/11 を標準入口にし、組織ポリシー / ライセンスで Docker Desktop や WSL2 が使えない端末は beginner guide のサポート境界外として明示する方針にしました。加えて tokoroten / nishio の「単一実行バイナリ」議論を受け、Node runtime を build-time assets に閉じ込めて Python/FastAPI + static assets runtime へ寄せる前提 issue `#885` を起票。MVP は external API route と、軽量モデル同梱で API 契約不要にする offline route の 2 本比較に修正しました。[[issue-877-windows-setup-guide-scope]]より [[node-runtime-free-windows-exe-2026-05-31]]より
...
- tokoroten / nishio の 2026-05-31 Slack で「Windows ユーザには実行バイナリ 1 個が嬉しい」→「Node を runtime 同梱せず、frontend を SPA/static assets にし、server-side wrapper を Python へ寄せる」案が出た。current main の `apps/admin` / `apps/static-site-builder` は薄い Node wrapper が多いため、段階的には可能と判断し、前提 issue `#885` を起票。nishio 指摘で MVP は external API route と offline bundled-model route の 2 本比較に修正。次は `apps/admin` の server actions / route handlers を client fetch or FastAPI へ寄せる feasibility sliceと、同梱モデル route の候補・速度・品質評価。[[node-runtime-free-windows-exe-2026-05-31]]より
