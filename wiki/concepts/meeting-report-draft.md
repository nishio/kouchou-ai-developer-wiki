...
- `PR #887` / Issue `#886`: 2026-06-01 に merge され Azure Deployment workflow も success になったが、本番 stable URL は旧 CSP のままで `.no-webgl` overlay も visible。Actions log では `latestRevisionName` は `public-viewer--0000166` まで進んだが `latestReadyRevisionName` は旧 `public-viewer--0000163` のままで、deploy confirmation が stable root の 200 を見て成功扱いにしていた。次は ACA revision logs / status で `0000166` が ready にならない原因を確認し、workflow 側も latest revision readiness と representative report smoke を見るように直す。詳細は [[issue-887-scattergl-csp-regression-2026-06-01]]。
