...
この回では、`#887` の本番 deploy false positive / OOM、open issue 全件棚卸し、PR #883 撤回後の developer-quickstart 再設計、組織内デモ役と SaaS/Azure 体験環境の扱い、Windows/ローカルLLM route が主な共有・相談事項だった。[[meeting-minutes]]より
...
5. 現在の open PR は `#863` の 1 本です。`#887` は merge され、public-viewer は一時 `public-viewer--0000166` の startup `next build` が TypeScript phase で `Killed` / exit 137 になり Ready になりませんでしたが、2026-06-01 19:44 JST 時点では self-recover して stable URL も `unsafe-eval` 付き CSP を返しています。deploy confirmation は実例として少なくとも `#821` (2026-04-11) から旧 ready revision の 200 で success になっており、今回だけの regression ではありません。workflow には latest revision readiness と report smoke が必要です。`#863` は `#731` Windows setup 文字化け対応で実機確認と review 判断待ち。`PR #883` は 2026-05-31 に撤回済みで、新方針による全文草案を [[pr-883-developer-quickstart-draft-2026-05-31]] に置き、Issue `#876` 本体にも追加要件と完了条件を反映済み。
...
- `PR #887` / Issue `#886`: 2026-06-01 に merge され Azure Deployment workflow も success になったが、当初 stable URL は旧 CSP のままで `.no-webgl` overlay も visible。`public-viewer--0000166` は startup `next build` の TypeScript phase で `Killed` / exit 137 を起こし、一時 `Unhealthy / Degraded` だった。その後 2026-06-01 19:41 JST に `next start` まで到達し、19:44 JST 確認時点では latestReady も `0000166`、stable URL も `unsafe-eval` 付き CSP。deploy success false positive は `#887` 固有ではなく、実例として少なくとも `#821` (2026-04-11) から successful deploy で `latestReadyRevisionName` が旧 revision のままでも stable URL 200 で success になっていた。`#785` 時点の workflow も stable URL curl 判定で、latest revision readiness は見ていなかった。workflow 側は latest revision readiness と representative report smoke を見るように直す必要がある。runtime build exit 137 risk は残るため、memory increase または image build 時 `.next` 化も検討。詳細は [[issue-887-scattergl-csp-regression-2026-06-01]]。
