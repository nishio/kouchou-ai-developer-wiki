...
2025-10-08 週では `embeddings.pkl` が元 embedding ではなく UMAP 後 2D であることが明示され、「実験したいなら embedding からやり直さないといけない」と書かれている。  
これは [[pipeline]] の hidden assumption と一致する。
...
## 3. SenseMaker / Jigsaw は「高級オプション」として意識されていた

[[slack-dev-kouchouai-2025-q4]] 2025-10-15 週では、Jigsaw SenseMaker 的に時間をかけてよいなら HDBSCAN に戻すのも手、という発言があり、同週で「お金と時間をかけていいユーザならオプションとして選べるようにするのも手」と整理される。

同 source 2025-11-26 週では、「いい感じの切り口を発見してから、その切り口に合わせて分類し直す」方が筋がいいとされている。  
この時点の SenseMaker / Jigsaw 参照は、2026-Q1 のような具体的 plugin 構造ではまだないが、**現行散布図方式の外側にある別分析モード** としてすでに強く意識されていた。
...
- 2025 4Q 時点では Jigsaw / SenseMaker は方向性として語られているが、互換枝や taxonomy-guided 亜種の具体化は 2026-Q1 を待つ
