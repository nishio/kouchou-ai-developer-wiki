...
tokoroten は、現代なら `TTTC Turbo` などの LLM 直接分類` のように LLM で直接分類した方が精度は高いだろうと述べる。  
これ自体は後の [[llm-grouping-background-history]] や `analysis_mode=llm_grouping` の方向と整合する。[[slack-niizuma-umap-kmeans-thread-2026-03-18]]より
...
後続の `LLM grouping` 実験は、この読みをかなり補強している。`LLM grouping` は top-level ラベル品質では強い一方、embedding 由来の 2D 散布図とは相性が悪く、scatter 指標では従来 hierarchical に負けた。つまり「意味的に良い分類」と「散布図として自然に見える配置」は別々に評価すべきで、1 つの algorithm score に畳み込むと設計判断を誤る。[[llm-grouping-experiment-output-2026-05-25]]より
