...
- `#874` は、named layout artifact を first-class にする意味では筋があるが、default workflow に常時 step を足す形が妥当かは要検討。少なくとも CI failure を直し、`layout_generation` を「表示 artifact の生成」として明確に位置づける必要がある
